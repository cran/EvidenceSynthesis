...
plotCovariateBalances <- function(balances,
  ...
  plot <- ggplot2::ggplot(balance, ggplot2::aes(x = .data$y, group = .data$label)) +
    ...
    ggplot2::geom_boxplot(ggplot2::aes(ymin = .data$ymin,
                                       lower = .data$lower,
                                       middle = .data$middle,
                                       upper = .data$upper,
                                       ymax = .data$ymax), outlier.shape = NA, data = agg, stat = "identity", alpha = 0.65) +
...
preparePsPlot <- function(data, unfilteredData = NULL, scale = "preference") {
  ...
  if (scale == "preference") {
    ...
    d <- data.frame(preferenceScore = c(d1$x, d0$x),
                    y = c(d1$y, d0$y),
    ...
    d <- data.frame(propensityScore = c(d1$x, d0$x),
                    y = c(d1$y, d0$y),
                    treatment = c(rep(1, length(d1$x)), rep(0, length(d0$x))))
    d$y <- d$y/max(d$y)
  }
